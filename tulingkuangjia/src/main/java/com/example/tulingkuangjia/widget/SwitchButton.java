...
public class SwitchButton extends CompoundButton {
	...
	public SwitchButton(Context context, AttributeSet attrs, int defStyle) {
		...
		mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(
				R.styleable.SwitchButton_thumb_margin,
		...
		mConf.setThumbWidthAndHeightInPixel(ta.getDimensionPixelSize(
				R.styleable.SwitchButton_thumb_width, -1), ta
				.getDimensionPixelSize(R.styleable.SwitchButton_thumb_height,
						-1));

		mConf.setMeasureFactor(ta.getFloat(
				R.styleable.SwitchButton_measureFactor, -1));
		...
		int velocity = ta.getInteger(
				R.styleable.SwitchButton_animationVelocity, -1);
	...
	private void initView() {
		mConf = Configuration.getDefault(getContext().getResources()
	...
	private void fetchDrawableFromAttr(TypedArray ta) {
		...
		mConf.setOffDrawable(fetchDrawable(ta,
				R.styleable.SwitchButton_offDrawable,
				R.styleable.SwitchButton_offColor,
				Configuration.Default.DEFAULT_OFF_COLOR));
		mConf.setOnDrawable(fetchDrawable(ta,
				R.styleable.SwitchButton_onDrawable,
				R.styleable.SwitchButton_onColor,
				Configuration.Default.DEFAULT_ON_COLOR));
		mConf.setThumbDrawable(fetchThumbDrawable(ta));
	}

	private Drawable fetchDrawable(TypedArray ta, int attrId, int alterColorId,
			int defaultColor) {
		...
		if (tempDrawable == null) {
			...
			((GradientDrawable) tempDrawable).setCornerRadius(this.mConf
					.getRadius());
	...
	private Drawable fetchThumbDrawable(TypedArray ta) {
		...
		Drawable tempDrawable = ta
				.getDrawable(R.styleable.SwitchButton_thumbDrawable);
		if (tempDrawable != null) {
			return tempDrawable;
		}
	...
	private void setupSafeZone() {
		...
		if (w > 0 && h > 0) {
			...
			left = getPaddingLeft()
					+ (mConf.getThumbMarginLeft() > 0 ? mConf
							.getThumbMarginLeft() : 0);
			right = w
					- getPaddingRight()
					- (mConf.getThumbMarginRight() > 0 ? mConf
							.getThumbMarginRight() : 0) + (-mConf.getShrinkX());
			top = getPaddingTop()
					+ (mConf.getThumbMarginTop() > 0 ? mConf
							.getThumbMarginTop() : 0);
			bottom = h
					- getPaddingBottom()
					- (mConf.getThumbMarginBottom() > 0 ? mConf
							.getThumbMarginBottom() : 0)
					+ (-mConf.getShrinkY());
	...
	private void setupBackZone() {
		...
		if (w > 0 && h > 0) {
			...
			int left, right, top, bottom;
			left = getPaddingLeft()
					+ (mConf.getThumbMarginLeft() > 0 ? 0 : -mConf
							.getThumbMarginLeft());
			right = w
					- getPaddingRight()
					- (mConf.getThumbMarginRight() > 0 ? 0 : -mConf
							.getThumbMarginRight()) + (-mConf.getShrinkX());
			top = getPaddingTop()
					+ (mConf.getThumbMarginTop() > 0 ? 0 : -mConf
							.getThumbMarginTop());
			bottom = h
					- getPaddingBottom()
					- (mConf.getThumbMarginBottom() > 0 ? 0 : -mConf
							.getThumbMarginBottom()) + (-mConf.getShrinkY());
	...
	private void setupThumbZone() {
		...
		if (w > 0 && h > 0) {
			...
			int left, right, top, bottom;
			left = mIsChecked ? (mSafeZone.right - mConf.getThumbWidth())
					: mSafeZone.left;
	...
	public void slideToChecked(boolean checked) {
		...
		int to = checked ? mSafeZone.right - mConf.getThumbWidth()
				: mSafeZone.left;
