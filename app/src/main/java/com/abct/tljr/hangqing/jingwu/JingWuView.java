...
public class JingWuView {
    private SimpleDateFormat mSimpleDateFormat;
    private TextView tljr_rankview_time;
    private List<List<Integer>> doubleList;
	...
	public JingWuView(Activity activity){
		this.activity=activity;
		mSimpleDateFormat=new SimpleDateFormat("yyyy-MM-dd");
		ListDataModel=new ArrayList<List<RankViewDataModel>>();
        doubleList=new ArrayList<List<Integer>>();
	...
	public void initView(){
		 rankviewpager = LayoutInflater.from(activity).inflate(R.layout.tljr_rankview_jingwu, null);
         tljr_rankview_time = (TextView) rankviewpager.findViewById(R.id.tljr_rankview_time);
         new UpdateRankViewData().execute();
//       Timer timer=new Timer();
// 		 timer.schedule(new update(),0,2000);
	}
	...
	@SuppressLint("InflateParams")
	public GridView flushJingWuItem(List<RankViewDataModel> ListDataModel,int page){
        ...
        RankViewDataModel mRankViewDataModel=null;
        for(int i=0;i<ListDataModel.size()+1;i++){
            ...
            if(i==0){
           	...
           	mRankViewDataModel=ListDataModel.get(i-1);
               if(editor.getString(mRankViewDataModel.getTimes()+mRankViewDataModel.getCode(),"").equals(mRankViewDataModel.getCode())){
               	...
               	((TextView)view.findViewById(R.id.rankview_code)).setText(mRankViewDataModel.getName()+"("+mRankViewDataModel.getCode()+")");
               	...
               	//如果没有挂过
               	 view=LayoutInflater.from(activity).inflate(R.layout.tljr_rankview_item,null);
                 View guagua=LayoutInflater.from(activity).inflate(R.layout.tljr_rankview_item_gua,null);
                 GuaGuaKa gua=(GuaGuaKa)guagua.findViewById(R.id.tljr_rankview_guagua);
                 gua.setUrl(mRankViewDataModel.getIconUrl(),mRankViewDataModel.getName());
                 ViewFlipper mViewFlipper=(ViewFlipper)view.findViewById(R.id.tljr_rankview_viewflipper);
                 gua.setViewFlipper(mViewFlipper,activity,ListDataModel,i-1,mViewPager,
                		 ListDataModel.get(0).getTimes(),view,mRankViewDataModel.getName());
                 mViewFlipper.addView(guagua,0);
               }
	...
	final Handler mHandler = new Handler() {
       ...
       public void handleMessage(Message msg) {
            ...
            case 1:
                if(mViewPager.getAdapter()==null){
                    ...
                    mViewPager.setOnPageChangeListener(new OnPageChangeListener() {
                        ...
                        public void onPageSelected(int page) {
                            if(doubleList.size()>(page)){
                                tljr_rankview_time.setText("("+doubleList.get(page).get(2)+"/"
                                        +doubleList.get(page).get(1)+"/"+doubleList.get(page).get(0)+"推荐)");
                            }
                ...
                tljr_rankview_time.setText("("+doubleList.get(0).get(2)+"/"+doubleList.get(0).get(1)
                        +"/"+doubleList.get(0).get(0)+"推荐)");
               break;
            case 2:
//           	ProgressDlgUtil.stopProgressDlg();
            ...
            case 3:
           	 for(String key:resultMap.keySet()){
        		 ...
        		 if (num > 0) {
        			//((TextView)view.findViewById(R.id.rankview_percent)).setTextColor(activity.getResources().getColor(R.color.red));
//        				((RelativeLayout) view.findViewById(R.id.rankview_header))
//        						.setBackgroundColor(activity.getResources().getColor(R.color.red));
        		 } else {
        		   //((TextView)view.findViewById(R.id.rankview_percent)).setTextColor(activity.getResources().getColor(R.color.tljr_c_green));
//        				((RelativeLayout) view.findViewById(R.id.rankview_header))
//        						.setBackgroundColor(activity.getResources().getColor(R.color.tljr_c_green));
        		 }
           	   }
           	 break;
            }
