...
public class SeekBarPressure extends View {
	...
	public SeekBarPressure(Context context, AttributeSet attrs, int defStyle) {
		...
		notScrollBarBg = resources
				.getDrawable(R.drawable.tljr_shape_progressbj);
		hasScrollBarBg = resources.getDrawable(R.drawable.tljr_shape_progress);
		// notScrollBarBg = resources
		// .getDrawable(R.drawable.img_fengexian1);
		// hasScrollBarBg = resources
		// .getDrawable(R.drawable.img_fengexian1);
		mThumbLow = resources.getDrawable(R.drawable.t2);
		mThumbHigh = resources.getDrawable(R.drawable.t1);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mOffsetLow = formatDouble(defaultScreenLow / 100 * (mDistance))
				+ mThumbWidth / 2;
		mOffsetHigh = formatDouble(defaultScreenHigh / 100 * (mDistance))
				+ mThumbWidth / 2;
	...
	protected void onDraw(Canvas canvas) {
		...
		int aaa = mThumbMarginTop + mThumbHeight - mScollBarHeight / 2;
		int bbb = aaa + mScollBarHeight;

		// 底，不会动
		notScrollBarBg.setBounds(mThumbWidth / 2, aaa, mScollBarWidth
				- mThumbWidth / 2, bbb);
		...
		// 后滑块
		mThumbHigh.setBounds((int) (mOffsetHigh - mThumbWidth / 2),
				mThumbMarginTop, (int) (mOffsetHigh + mThumbWidth / 2),
				mThumbHeight + mThumbMarginTop);
		mThumbHigh.draw(canvas);

		// 前滑块
		mThumbLow.setBounds((int) (mOffsetLow - mThumbWidth / 2),
				mThumbMarginTop, (int) (mOffsetLow + mThumbWidth / 2),
				mThumbHeight + mThumbMarginTop);
		mThumbLow.draw(canvas);
		double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) * 100
				/ mDistance);
		double progressHigh = formatDouble((mOffsetHigh - mThumbWidth / 2)
				* 100 / mDistance);
		...
		//
		// // 蓝色，中间部分会动
		// hasScrollBarBg.setBounds((int) mOffsetLow, aaa, (int) mOffsetHigh,
		// bbb);
		// hasScrollBarBg.draw(canvas);
		//
		// // 前滑块
		...
		// mThumbHigh.draw(canvas);
		//
		// double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) *
		// 100
		// / mDistance);
		// double progressHigh = formatDouble((mOffsetHigh - mThumbWidth / 2)
		// * 100 / mDistance);
		// // Log.d(TAG, "onDraw-->mOffsetLow: " + mOffsetLow +
		// "  mOffsetHigh: " +
		// // mOffsetHigh + "  progressLow: " + progressLow + "  progressHigh: "
		// +
		// // progressHigh);
		// // canvas.drawText((int) progressLow + "", (int) mOffsetLow - 2 - 2,
		// 15,
		// // text_Paint);
		// // canvas.drawText((int) progressHigh + "", (int) mOffsetHigh - 2,
		// 15,
		// // text_Paint);
		//
		// if (mBarChangeListener != null) {
			...
			if (mFlag == CLICK_ON_LOW) {
				if (e.getX() < 0 || e.getX() <= mThumbWidth / 2) {
					...
					if (mOffsetHigh - mOffsetLow <= 0) {
						mOffsetHigh = (mOffsetLow <= mDistance + mThumbWidth
								/ 2) ? (mOffsetLow)
				...
				if (e.getX() < mThumbWidth / 2) {
					...
					mOffsetHigh = formatDouble(e.getX());
					if (mOffsetHigh - mOffsetLow <= 0) {
						mOffsetLow = (mOffsetHigh >= mThumbWidth / 2) ? (mOffsetHigh)
								: mThumbWidth / 2;
					}
				}
			}
	...
	public int getAreaFlag(MotionEvent e) {
		...
		if (e.getY() >= top && e.getY() <= bottom
			...
			return CLICK_ON_HIGH;
				...
				&& e.getY() <= bottom
				&& ((e.getX() >= 0 && e.getX() < (mOffsetLow - mThumbWidth / 2)) || ((e
						.getX() > (mOffsetLow + mThumbWidth / 2)) && e.getX() <= ((double) mOffsetHigh + mOffsetLow) / 2))) {
			return CLICK_IN_LOW_AREA;
		} else if (e.getY() >= top
				&& e.getY() <= bottom
				&& (((e.getX() > ((double) mOffsetHigh + mOffsetLow) / 2) && e
						.getX() < (mOffsetHigh - mThumbWidth / 2)) || (e.getX() > (mOffsetHigh + mThumbWidth / 2) && e
						.getX() <= mScollBarWidth))) {
	...
	public void setProgressLow(double progressLow) {
		...
		mOffsetLow = formatDouble(progressLow / 100 * (mDistance))
				+ mThumbWidth / 2;
		isEdit = true;
		refresh();
	}
	...
	public void setProgressHigh(double progressHigh) {
		...
		mOffsetHigh = formatDouble(progressHigh / 100 * (mDistance))
				+ mThumbWidth / 2;
		isEdit = true;
		refresh();
	}
