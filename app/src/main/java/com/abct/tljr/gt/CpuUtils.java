...
public class CpuUtils {
	...
	public static double getCpuUsage() {
		...
		if (initCpu) {
			...
			try {
				...
				o_cpu = Double.parseDouble(toks[2]) + Double.parseDouble(toks[3]) + Double.parseDouble(toks[4])
						+ Double.parseDouble(toks[6]) + Double.parseDouble(toks[8]) + Double.parseDouble(toks[7]);
			...
			try {
				...
				double c_cpu = Double.parseDouble(toks[2]) + Double.parseDouble(toks[3]) + Double.parseDouble(toks[4])
						+ Double.parseDouble(toks[6]) + Double.parseDouble(toks[8]) + Double.parseDouble(toks[7]);
				if (0 != ((c_cpu + c_idle) - (o_cpu + o_idle))) {
					// double value = (100.00 * ((c_cpu - o_cpu) ) / ((c_cpu +
					// c_idle) - (o_cpu + o_idle)));
					usage = DoubleUtils.div((100.00 * ((c_cpu - o_cpu))), ((c_cpu + c_idle) - (o_cpu + o_idle)), 2);
	...
	public static double getCpuUsage0() {
		try {
			...
			double cpu1 = Double.parseDouble(toks[2]) + Double.parseDouble(toks[3]) + Double.parseDouble(toks[4])
					+ Double.parseDouble(toks[6]) + Double.parseDouble(toks[8]) + Double.parseDouble(toks[7]);
			...
			double cpu2 = Double.parseDouble(toks[2]) + Double.parseDouble(toks[3]) + Double.parseDouble(toks[4])
					+ Double.parseDouble(toks[6]) + Double.parseDouble(toks[8]) + Double.parseDouble(toks[7]);
			double value = DoubleUtils.div((100.00 * ((cpu2 - cpu1))), (cpu2 + idle2) - (cpu1 + idle1), 2);
			// BigDecimal b = new BigDecimal(Double.toString(value));
